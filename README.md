...
<p align="center">
  <img src="assets/telegram.jpg" alt="Boop replying inside Telegram" width="320" />
  <br>
  <sub><em>Boop in action — message it like a person, get back an answer with full context.</em></sub>
</p>
...
```
 Telegram  →  long-polling (getUpdates)  →  Interaction agent  →  Sub-agents (per task)
                                                   │                    │
                                                   ▼                    ▼
...
- [Telegram Bot API](https://core.telegram.org/bots/api) — Telegram in/out via long-polling (free)
...
- **Telegram in / Telegram out** via the Bot API (with typing indicators and offset-based dedup).
- **No webhook needed** — Boop uses long-polling (`getUpdates`) so there's no public URL required just for messaging.
...
If you want to see what it looked like before I transitioned to a Telegram-based agent, here's [the walkthrough on YouTube](https://www.youtube.com/watch?v=_h2EnRfxMQE).
...
| [Telegram](https://telegram.org/) | Messaging. Create a bot via @BotFather, grab your bot token. No phone provisioning needed. | Free | — |
...
# 4. Start everything with one command — server, Convex, and debug UI
npm run dev
```

`npm run dev` prints color-prefixed output from all three processes and shows a banner once everything is live.
...
Boop uses long-polling (`getUpdates`) — **no public URL or ngrok needed** just for messaging. Simply message your bot on Telegram and the agent replies.

> **Note:** If you use Composio integrations, Composio handles its own OAuth callbacks on `platform.composio.dev`, so no tunnel is needed for that either. A `PUBLIC_URL` in `.env.local` is only needed if you have a custom integration that requires an inbound webhook of your own.

> **Gotcha:** `BOOP_USER_CHAT_ID` must be your Telegram chat ID (the numeric ID of the conversation where you message the bot). You can get it by messaging [@userinfobot](https://t.me/userinfobot) on Telegram.
>
> **Fix in one command:** `npm run telegram:check` polls for a pending message and prints your chat ID.

---

## How the Telegram integration works

Boop connects to Telegram via the Bot API using long-polling (`getUpdates`). There is no webhook to register — the server continuously polls for new messages and processes them as they arrive. Two NPM scripts are available:

| Command | What it does |
|---|---|
| `npm run setup` | Interactive. Prompts for your `TELEGRAM_BOT_TOKEN` (from @BotFather) and `BOOP_USER_CHAT_ID` and writes them to `.env.local`. |
| `npm run telegram:check` | Polls Telegram once for any pending messages and prints the sender's chat ID — useful when you need to look up your chat ID. |

### The `npm run dev` lifecycle

```
 1. Preflight: confirm convex/_generated/ exists (else prompt to run setup).
 2. Spawn three children in parallel, each with a prefixed log stream:
       server │   (tsx watch server/index.ts)
       convex │   (npx convex dev — pushes schema + functions)
       debug  │   (vite dev server on :5173)
 3. Wait for all three readiness signals:
       server → "listening on :PORT"
       convex → "Convex functions ready"
       debug  → "Local:  http://localhost:5173/"
 4. Server starts long-polling Telegram — no webhook registration needed.
 5. Show the banner with dashboard URL and bot username.
```
...
```
════════════════════════════════════════════════════════════════════
  Boop is ready — Telegram long-polling active.
...
When you message your bot, expect this sequence in your terminal:
...
- **`[telegram]`** — outbound send results. If the Bot API rejects, the error body is logged with a hint about the likely cause (invalid token, chat not found, etc.).
...
Conversation IDs use the format `telegram:{chat_id}` (e.g. `telegram:123456789`).

### When to re-run each Telegram script

- **First time / after losing `.env.local`** → `npm run setup` (walks through Telegram + Convex together)
- **Need to look up your chat ID** → `npm run telegram:check`

Visit `http://localhost:5173` for the debug dashboard (chat, agents, memory, events). You can also chat from the dashboard's Chat tab without Telegram.
...
| `BOOP_USER_CHAT_ID` | yes | Your Telegram chat ID. Message [@userinfobot](https://t.me/userinfobot) to find it, or run `npm run telegram:check`. |
...
| `PUBLIC_URL` | no | Only needed if you have a custom integration that requires an inbound webhook. Composio handles its own OAuth callbacks on `platform.composio.dev`. Long-polling means no public URL is required for Telegram messaging. |
...
│   ├── telegram.ts                # Telegram long-polling, reply, typing indicator
...
│   └── telegram-check.mjs         # Polls Telegram once and prints incoming chat IDs
...
- Check that `TELEGRAM_BOT_TOKEN` is set correctly in `.env.local` (get one from @BotFather).
...
- The server exposes `POST /chat` with `{ conversationId, content }` — curl or a tiny client can drive the agent directly, no Telegram required.

**Claude SDK says no credentials.**
- Run `claude` once and sign in, or set `ANTHROPIC_API_KEY` in `.env.local`.

**"Chat not found" / Telegram API errors.**
- `BOOP_USER_CHAT_ID` may be wrong or unset. Run `npm run telegram:check` to poll for a pending message and print the correct chat ID, then update `.env.local`.
