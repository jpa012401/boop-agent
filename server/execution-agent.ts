...
import { draftStagingTools } from "./tools/drafts.js";
...
export async function spawnExecutionAgent(opts: SpawnOptions): Promise<SpawnResult> {
  ...
  const execTools = [...executionTools, ...(opts.conversationId ? draftStagingTools : [])];
  const mcpServers: Record<string, McpSdkServerConfigWithInstance> = {
    ...integrationServers,
    "boop-exec-tools": toolSpecsToClaudeMcp("boop-exec-tools", "0.1.0", execTools, execCtx),
  };
